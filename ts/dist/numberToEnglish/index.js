...
exports.numberToEnglish = void 0;
const numbers = {
    0: "zero",
    1: "one",
    2: "two",
...
function numberToEnglish(x) {
    if (x == 0)
        return "zero";
    else {
        let tens = { 1: 0, 10: 0, 100: 0, 1000: 0 };
        let res = [];
        let str = x.toString();
        for (let i = 0; i < str.length; i++) {
            if (i < 4)
                tens[10 ** i] = Number(str[str.length - i - 1]);
            else
                tens[10 ** 3] = Number(str.substring(0, str.length - i) + tens[10 ** 3]);
        }
        for (const key in tens) {
            if (Object.prototype.hasOwnProperty.call(tens, key) && tens[key] !== 0) {
                ...
                if (key == "10" && e == 1) {
                    ...
                    res.unshift(numbers[e] + " " + numbers[key]);
            }
        }
        return res.join(" ");
    }
