...
exports.G964 = G964;
_a = G964;
G964.towns = (town, strng) => {
    const townsObj = {};
    let forecasts = String(strng).split("\n");
    for (let i = 0; i < forecasts.length; i++) {
        let t = forecasts[i].split(":")[0];
        townsObj[t] = forecasts[i]
            .split(":")[1]
            .split(",")
            .map((e) => Number(e.split(" ")[1]));
    }
    return townsObj[town];
};
G964.mean = (town, strng) => {
    // return this.towns.reduce((a: number, b: number) => a + b) / this.towns.length
};
G964.variance = (town, strng) => {
    _a.mean;
};
